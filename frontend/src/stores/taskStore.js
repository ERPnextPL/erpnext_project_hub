	...
	async function createTask(taskData) {
		try {
			const data = await apiCall(
				"erpnext_projekt_hub.api.project_hub.create_task",
				taskData
			);
			if (data) {
			...
			if (data) {
				...
				if (updates.status === "Completed" && data.status !== "Completed") {
					...
					if (window.frappe && incompleteSubtasks.length > 0) {
						const subtaskNames = incompleteSubtasks
							.slice(0, 3)
							.map((t) => t.subject)
							.join(", ");
						const moreCount =
...
async function reorderTask(taskName, newParent, newIdx) {
	...
	async function fetchProjectsSettings() {
		try {
			const data = await apiCall(
				"erpnext_projekt_hub.api.project_hub.get_projects_settings",
				{}
			);
	...
	async function fetchTaskStatuses() {
		try {
			const data = await apiCall(
				"erpnext_projekt_hub.api.project_hub.get_task_statuses",
				{}
			);
	...
	async function fetchTaskPriorities() {
		try {
			const data = await apiCall(
				"erpnext_projekt_hub.api.project_hub.get_task_priorities",
				{}
			);
	...
	async function fetchMilestones(projectName) {
		try {
			const data = await apiCall(
				"erpnext_projekt_hub.api.project_hub.get_project_milestones",
				{
					project: projectName,
				}
			);
	...
	async function createMilestone(milestoneData) {
		try {
			const data = await apiCall(
				"erpnext_projekt_hub.api.project_hub.create_milestone",
				milestoneData
			);
	...
	async function assignTaskToMilestone(taskName, milestoneName) {
		try {
			const data = await apiCall(
				"erpnext_projekt_hub.api.project_hub.assign_task_to_milestone",
				{
					task_name: taskName,
