...
const hasSubtasks = computed(() => (props.task.children?.length || 0) > 0);
...
<template>
	...
	>
		<!-- Header -->
		<div class="sticky top-0 z-30 flex items-center justify-between px-4 py-4 border-b border-gray-200 bg-white">
		...
		<div class="flex-1 overflow-y-auto">
			<div class="p-4 space-y-4">
				<section class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
					...
					<button
						type="button"
							class="flex items-center justify-between w-full text-left text-sm font-semibold leading-tight text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1"
						...
						>
							...
							<div>
								<input
									...
									type="text"
										class="w-full text-lg font-semibold leading-tight text-gray-900 border-0 p-0 focus:ring-0 placeholder:text-gray-400"
					...
					>
						...
						>
							...
							<div class="flex items-center gap-3">
									<label class="text-sm leading-tight text-gray-500 w-20">{{ translate("Expected Time") }}</label>
								<input
							...
							<div class="flex items-center gap-3">
									<label class="text-sm leading-tight text-gray-500 w-20">{{ translate("Start Date") }}</label>
								<input
							...
							<div class="flex items-center gap-3">
									<label class="text-sm leading-tight text-gray-500 w-20">{{ translate("Progress") }}</label>
								<div class="flex-1 flex items-center gap-2">
									<input
										...
										class="flex-1"
										:class="hasSubtasks ? 'opacity-50 cursor-not-allowed' : ''"
									/>
							...
							<div v-if="hasSubtasks" class="text-xs leading-tight text-gray-500 pl-20">
								{{ translate("Progress is controlled by subtasks.") }}
							</div>

							<div>
								<div class="flex items-center justify-between">
									<label class="text-sm font-medium leading-tight text-gray-700 mb-2 block">{{
		...
		<div class="border-t border-gray-200 px-4 py-3">
			<div class="flex items-center justify-between text-xs text-gray-500">
				<div class="flex items-center gap-3">
					<span v-if="task.creation">Utworzono {{ formatTime(task.creation) }}</span>
					<span
