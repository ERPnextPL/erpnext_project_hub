  ...
  const fillMetaDetails = (card, $container) => {
    ...
    if (assignees.length) {
      ...
      assignees.slice(0, 6).forEach((assignee) => {
        ...
        const $avatar = $(`<span class='project-hub-avatar' title='${safeAssignee}'>${initials}</span>`);
        $list.append($avatar);
      });
  ...
  const showTaskPreviewDialog = (task) => {
    ...
    const dialog = new frappe.ui.Dialog({
      ...
      fields: [
        ...
        { fieldname: "description", fieldtype: "Text Editor", label: __("Description"), read_only: 1 },
        { fieldtype: "Column Break" },
        { fieldname: "status", fieldtype: "Link", options: "Task Status", label: __("Status"), read_only: 1 },
        { fieldname: "priority", fieldtype: "Link", options: "Task Priority", label: __("Priority"), read_only: 1 },
        { fieldtype: "Section Break", label: __("Dates & Assignment") },
        { fieldname: "exp_end_date", fieldtype: "Date", label: __("End Date"), read_only: 1 },
        { fieldtype: "Column Break" },
        { fieldname: "_assign", fieldtype: "MultiSelectList", options: "User", label: __("Assignees"), read_only: 1 },
        { fieldtype: "Section Break", label: __("Project Details") },
        { fieldname: "project", fieldtype: "Link", options: "Project", label: __("Project"), read_only: 1 },
        { fieldtype: "Column Break" },
        { fieldname: "progress", fieldtype: "Percent", label: __("Progress"), read_only: 1 },
      ],
      size: "large",
